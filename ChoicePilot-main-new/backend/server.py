...
async def confirm_email(token: str):
    ...
    return await email_verification_service.verify_email_token(token)
    ...
    try:
        ...
        if request.step == "initial":
            ...
            followup = await generate_followup_question(
                request.message, 1, session.get("category")
            )
    ...
    try:
        ...
        if not session and request.step == "initial":
            ...
            await db.decision_sessions_advanced.update_one(
                ...
                {
                    "$set": {
                        "followup_questions": question_dicts,  # Store as dictionaries
                        "current_step": "followup",
            ...
            return AdvancedDecisionStepResponse(
                ...
                followup_questions=[enhanced_questions[0]]
                if enhanced_questions
                else [],  # Only first question
                decision_type=decision_type.value,
                session_version=1,
            )
...
async def debug_ai_orchestrator():
    ...
    return {
        ...
        "llm_router_available": hasattr(ai_orchestrator, "llm_router")
        if ai_orchestrator
        else False,
        "classifier_available": hasattr(ai_orchestrator, "classifier")
        if ai_orchestrator
        else False,
        "smart_router_available": hasattr(ai_orchestrator, "smart_router")
        if ai_orchestrator
        else False,
        "followup_engine_available": hasattr(ai_orchestrator, "followup_engine")
        if ai_orchestrator
        else False,
    }
