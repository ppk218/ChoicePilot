...
class AccountSecurityService:
    ...
    async def export_user_data(self, user_id: str, export_format: str = "json") -> dict:
        ...
        try:
            ...
            decisions = await self.db.decision_sessions.find({"user_id": user_id}).to_list(None)
            conversations = await self.db.conversations.find({"user_id": user_id}).to_list(None)
            ...
            shares = await self.db.decision_shares.find({"user_id": user_id}).to_list(None)
            ...
            export_data = {
                ...
                "summary": {
                    ...
                    "last_active": max([d.get("last_active", datetime.min) for d in decisions] + [datetime.min])
                }
            }
    ...
    async def delete_user_account(self, user_id: str, password: str, confirmation: str) -> dict:
        ...
        try:
            ...
            deletion_results["decisions"] = await self.db.decision_sessions.delete_many({"user_id": user_id})
            deletion_results["conversations"] = await self.db.conversations.delete_many({"user_id": user_id})
            
            # Delete payment and subscription data
            deletion_results["payments"] = await self.db.payments.delete_many({"user_id": user_id})
            deletion_results["subscriptions"] = await self.db.subscriptions.delete_many({"user_id": user_id})
            
            # Delete shared decisions
            deletion_results["shares"] = await self.db.decision_shares.delete_many({"user_id": user_id})
            
            # Delete email verifications
            deletion_results["verifications"] = await self.db.email_verifications.delete_many({"email": user["email"]})
            ...
            return {
                ...
                "deletion_summary": {
                    ...
                    "conversations_deleted": deletion_results["conversations"].deleted_count,
                    "payments_deleted": deletion_results["payments"].deleted_count,
                    "subscriptions_deleted": deletion_results["subscriptions"].deleted_count,
    ...
    def _clean_export_data(self, data: dict) -> dict:
        ...
        sensitive_fields = [
            "password_hash", "dodo_payment_id", "dodo_subscription_id",
            "payment_method", "webhook_signature"
        ]
    ...
    async def _log_account_deletion(self, user_id: str, email: str, deletion_results: dict):
